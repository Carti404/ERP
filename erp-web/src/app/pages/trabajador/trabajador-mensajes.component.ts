...
import {
  WORKER_MAILBOX_CONTACT_SUGGESTIONS,
  WORKER_MAILBOX_FOLDERS,
  WORKER_MAILBOX_MESSAGES,
  type WorkerMailboxContactSuggestion,
  type WorkerMailboxFolderId,
  type WorkerMailboxMessage,
} from './trabajador-mensajes.mock';

/** Destinatario en el modal de redacción (demo). */
interface WorkerComposeRecipient {
  readonly id: string;
...
@Component({
  ...
  standalone: true,
  templateUrl: './trabajador-mensajes.component.html',
})
export class TrabajadorMensajesComponent {
  protected readonly folders = WORKER_MAILBOX_FOLDERS;

  protected readonly folder = signal<WorkerMailboxFolderId>('inbox');

  protected readonly searchQuery = signal('');

  protected readonly selectedId = signal<string | null>('w1');

  protected readonly mobilePane = signal<'list' | 'reader'>('list');

  protected readonly composeOpen = signal(false);

  protected readonly composeRecipients = signal<WorkerComposeRecipient[]>([]);
  ...
  protected readonly listTitle = computed(() => {
    ...
    return this.folders.find((x) => x.id === id)?.label ?? 'Bandeja';
  });

  protected readonly filteredMessages = computed(() => {
    const q = this.searchQuery().trim().toLowerCase();
    const f = this.folder();
    let list = WORKER_MAILBOX_MESSAGES.filter((m) => {
      if (f === 'critical') {
  ...
  protected readonly selected = computed((): WorkerMailboxMessage | null => {
    const id = this.selectedId();
    if (!id) {
      return null;
    }
    return WORKER_MAILBOX_MESSAGES.find((m) => m.id === id) ?? null;
  });

  /** Atributo `data-reading-accent` en el panel lector (null = sin mensaje). */
  protected readonly readingAccentAttr = computed((): string | null => {
    const m = this.selected();
    return m ? (m.accent ?? 'general') : null;
  });

  /** Sugerencias filtradas por lo que escribe el usuario en “Para”. */
  protected readonly composeSuggestions = computed((): WorkerMailboxContactSuggestion[] => {
    ...
    let list = WORKER_MAILBOX_CONTACT_SUGGESTIONS.filter((c) => !chosenIds.has(c.id));
    if (!q) {
  ...
  protected setFolder(id: WorkerMailboxFolderId): void {
    this.folder.set(id);
  ...
  protected commitDraftRecipient(): void {
    ...
    const exact = WORKER_MAILBOX_CONTACT_SUGGESTIONS.find(
  ...
  protected pickComposeSuggestion(c: WorkerMailboxContactSuggestion): void {
  ...
  protected onToolbarAction(_action: 'reply' | 'forward' | 'archive' | 'delete'): void {
    return;
  }
  ...
  protected messageAccent(m: WorkerMailboxMessage): string {
